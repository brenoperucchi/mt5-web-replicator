  ...
  describe API::V2::APIStore do
    context 'APIStoreStore Config' do
      it 'Create AccountServer Name same STORE with different customer' do 
        ...
        expect(Account.find_by(name: 5647753, account_server_id:1).id).to be == 1
        expect(Account.find_by(name: 5647753, account_server_id:1).account_server.name).to be == "darwinexdemo"
        expect(Account.find_by(name: 5647753, account_server_id:1).account_server_id).to be == 1
        # expect(Account.find_by(name: 5647753, account_server_id:2).account_server.name).to be == "darwinexdemo"
        ...
        expect(Account.find_by(name: 5647753, account_server_id:2).id).to be == 2
        expect(Account.find_by(name: 5647753, account_server_id:1).account_server.name).to be == "darwinexdemo"
        expect(Account.where(name: 5647753, account_server_id:2).last.account_server.name).to be == "xpdemo"
        expect(Account.find_by(name: 5647753, account_server_id:2).account_server_id).to be == 2
      ...
      it 'Create AccountServer Name same STORE with different customer' do 
        ...
        expect(Account.find_by(name: 5647753, account_server_id:3).account_server.name).to be == "darwinexdemo"
        expect(Account.find_by(name: 5647753, account_server_id:4).account_server.name).to be == "xpdemo"
        expect(Account.find_by(name: 5647753, account_server_id:3).store_id).to be == 1
        expect(Account.find_by(name: 5647753, account_server_id:4).store_id).to be == 1
        ...
        post "/api/v2/stores/config/imentore_copy/2_10/DarwinexDemo/201002/HEDGING", params: {'EnvironmentLocal':'1'}
        expect(response).to have_http_status 201
        ...
        expect(Account.where(name: 201002, account_server_id:3, store_id:2).last.account_server.name).to be == "darwinexdemo"
        expect(Account.where(name: 201002, account_server_id:3).last.store_id).to be == 2
      end
    end
  end
