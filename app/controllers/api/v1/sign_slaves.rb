...
module API
  module V1
    class SignSlaves < Grape::API
      include API::V1::Defaults

      resource :sign_slaves do
        desc "Return all signs"
        get "" do
          # return "{\"id\":\"1\",\"provider\":\"-481414224\"}, / {\"id\":\"2\",\"provider\":\"-222222\"},"
          SignOrder.all.to_json(only: [:id, :provider]).delete('[]').gsub("},", "}, / ")
          foo = {'collection'=> SignOrder.all.collect{|x| {id: x.id.to_s}}}.to_json
