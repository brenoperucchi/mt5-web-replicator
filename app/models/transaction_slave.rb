class TransactionSlave < ApplicationRecord
	...
	scope :entire, 	 	->{where(state: ['pending','executed','remove'])}
	...
	def api_request_attributes
		instrument = transaction_master.order.trace.instruments.find_by_symbol(symbol)
		openprice = (ordertype == "0" or ordertype == 1) ? "0" : price_request
		msg = "#{ordertype}|#{ticket}|#{transaction_master.try(:order).try(:trace).try(:id)}|#{self.id}|#{self.magic_number}|#{transaction_master.id}|#{openprice}|#{lot}|#{stop_loss}|#{take_profit}|#{state}|#{instrument.name}|#{comment}|"
		return msg
	end
end